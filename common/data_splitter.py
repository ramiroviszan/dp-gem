...
class DataSplitter:
    ...
    def run(self):
        ...
        for dataset_name, dataset_desc in self.datasets.items():
            loaded[dataset_name] = data_utils.load_file(dataset_desc['original'], dataset_desc['to_read'], True, int)
        
        loaded = self.tweak_data_before_split(loaded)

        for dataset_name, dataset_desc in self.datasets.items():
            self._split_dataset(loaded[dataset_name], *list(dataset_desc.values())[2:]) #2 removes 'original' and 'to_read'
