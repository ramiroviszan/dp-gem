...
class Gen:
    ...
    def _get_model(self):
        ...
        try:
            self.model_diff = models.load_model_adapter(self.network_fullpath)
        except:
            print("\nModel", self.network_fullpath,
                  "not found. Training started...")
            self.model_diff = self._train_model(*self.network_params.values())
    ...
    def _train_model(self, model_type, model_params, train_sessions):
        ...
        model, model_diff = models.create_model(model_type, model_params.values())
    ...
    def _load_data_to_compare(self):
        t_sets = self.datasets_params['to_compare']
        self.datasets_to_compare = {}
        ...
        for dataset in self.datasets_to_compare:
            for seq in dataset:
                rep = np.random.
